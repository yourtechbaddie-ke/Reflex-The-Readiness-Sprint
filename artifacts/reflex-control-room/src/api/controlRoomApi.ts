...
async function request<T>(endpoint: string, options?: RequestInit): Promise<T> {
  ...
  if (!response.ok || !result?.success || result.data === undefined) {
    throw new Error(result?.error?.message || "The live operations data could not be loaded.");
  }
  return result.data;
}
...
export async function createControlRoomDelivery(data: {
  ...
  const result = await request<{ delivery: Delivery }>("/deliveries", {
    method: "POST",
...
export async function assignControlRoomRider(deliveryId: string, riderId: string): Promise<Delivery> {
  const result = await request<{ delivery: Delivery }>(`/deliveries/${deliveryId}/assign`, {
    method: "PATCH",
...
export async function updateControlRoomDeliveryStatus(
  ...
  const result = await request<{ delivery: Delivery }>(`/deliveries/${deliveryId}/status`, {
    method: "PATCH",
