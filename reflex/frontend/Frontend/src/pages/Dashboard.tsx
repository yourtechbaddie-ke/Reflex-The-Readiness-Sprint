...
function Dashboard() {
  ...
  const load = useCallback(async () => {
    if (!token) { setLoading(false); setError("Sign in as a dispatcher from Deliveries to enable live assignment controls."); return; }
    ...
    try {
      ...
      if (!selectedDelivery) setSelectedDelivery(nextDeliveries.find((d) => d.status === "PENDING")?.id || "");
  ...
  return <div className="dashboard-page">
    ...
    {error && <div className="error-state"><strong>Control Room notice</strong><span>{error}</span></div>}
    <section className="panel dispatch-action-panel"><div className="panel-header"><div><p className="eyebrow">Assignment action</p><h3>Assign a rider</h3><p className="panel-subtitle">Choose a pending delivery and an available rider.</p></div><span className="dispatch-action-icon">🚚</span></div><div className="dispatch-action-content"><div className="dispatch-detail-card"><div><span>Pending deliveries</span><strong>{pending.length}</strong></div><div><span>Available riders</span><strong>{availableRiders.length}</strong></div><div><span>Active deliveries</span><strong>{active.length}</strong></div></div><div className="form-grid"><label className="dispatch-select-field"><span>Delivery</span><select value={selectedDelivery} onChange={(e) => setSelectedDelivery(e.target.value)}><option value="">Choose a pending delivery</option>{pending.map((delivery) => <option key={delivery.id} value={delivery.id}>{delivery.id} · {delivery.deliveryAddress}</option>)}</select></label><label className="dispatch-select-field"><span>Available rider</span><select value={selectedRider} onChange={(e) => setSelectedRider(e.target.value)}><option value="">Choose a rider</option>{availableRiders.map((rider) => <option key={rider.id} value={rider.id}>{rider.name} · {rider.area || "Nairobi"}</option>)}</select></label></div><button className="primary-button dispatch-assign-button" type="button" disabled={!selectedDelivery || !selectedRider || saving} onClick={() => void handleAssign()}>{saving ? "Assigning…" : "Assign rider"}<span>→</span></button></div></section>
